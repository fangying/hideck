...
这两次真人测试先使用临时脚本验证生命周期。随后部署正式 Unix socket broker，已完成外部呼入接听、主动外呼各两次手机端听测，最新复测用户分别确认声音清晰、清楚。正式版本不再依赖临时日志监听脚本。主机重启、冷启动、短信和上网仍不能沿用旧版本的验收结论；手机到电脑的语音内容及长时间稳定性仍需单独核验。
...
- 新分支推送后再次外呼、再呼入，用户均确认测试语音清楚。分支构建程序与现场部署程序 SHA-256 一致；此次仅记录听测，不触发整机重启，不改动 main。
