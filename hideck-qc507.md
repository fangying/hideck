...
原常驻方案此前有温重启、上网和真人通话记录，但本轮更换了音频生命周期：**新 broker 版本仍需重新进行整机重启、外呼和真人双向回归**。不能把旧版的成功当作新版已完成验收。ADB 完全失联、USB 硬件或供电故障不保证免除现场维修。

## 本轮证据与边界

| 项目 | 当前证据 |
| --- | --- |
| 外部呼入，接听侧到手机 | 临时生命周期脚本下连续两次听测通过；随后正式 broker 版本再次呼入，用户确认听到测试语音 |
| 主动外呼，接听侧到手机 | 正式 broker 版本从 Edge 主动拨号，接通后播放测试语音，用户确认收到且有声音 |
...
- **不要夸大稳定性**：两次听测成功支持方案方向，但没有完成长时间、重启和全部双向内容验收；初始化通常还需数秒，失败必须显式反馈。
